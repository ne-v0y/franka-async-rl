...
class ReacherEnv(RTRLBaseEnv, gym.core.Env):
    ...
    def __init__(self,
        ...
        self._nothing_packet = np.zeros(self._actuator_comms['Franka'].actuator_buffer.array_len)
    ...
    def _read_sensation(self):
        ...
        print("joint sensation", joint_sensation)
    ...
    def _compute_joint_(self, name, sensor_window, timestamp_window, index_window):
        ...
        # q_target
        # print(self._q_)
        self._q_ = np.array([sensor_window[i][1][0] for i in range(index_start,index_end)])
    ...
    def _check_done(self):
        ...
        if (self._episode_steps >= self._episode_length_step):# or env_done:
            self._actuator_comms['Franka'].actuator_buffer.write(self._nothing_packet)
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        while not done:
            ...
            img, joint, rewards, done = env.step(action)
        epi += 1
